...
class AbstractView(TemplateView):
    ...
    def get_data(self,request):
        ...
        if form.is_valid():
            counter=self.Counter().count(form.cleaned_data.get('value'))                      #set data for couter
            self.context = {"Counter": counter, "form": Calculate()}                          #set data for view
            return render(request, self.template_name, self.context)                          #return view
        return render(request, self.template_name, self.context)                              #return view
